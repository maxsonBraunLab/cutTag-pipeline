...
```{r data_prep}
multiqc_data <- jsonlite::read_json(path = params$multiqc_json, simplifyVector = TRUE)
...
multiqc_rawdata_dt <- multiqc_rawdata_df |>
	...
	DT::formatRound(
		columns = c("Read pairs pre-filt", "Read pairs post-filt", "Mapped read pairs", "Peaks (all)", "Peaks (no blacklist)"),
		digits = 0) |>
	DT::formatRound(
